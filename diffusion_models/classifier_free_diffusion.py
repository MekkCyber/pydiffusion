...
from utils.schedulers import linear_beta_schedule, cosine_beta_schedule
from utils.utils import extract, normalize_to_neg_one_to_one, unnormalize_to_zero_to_one, identity

ModelPrediction =  namedtuple('ModelPrediction', ['pred_noise', 'pred_x_start'])

class GaussianDiffusion(nn.Module):
    def __init__(self, model,*, image_size, timesteps = 1000, sampling_timesteps = None, objective = 'pred_noise', beta_schedule = 'cosine', ddim_sampling_eta = 1.,
    ...
    def model_predictions(self, x, t, classes, condition_scale = 6., rescaled_phi = 0.7, clip_x_start = False):
        model_output = self.model.forward_with_condition_scale(x, t, classes, condition_scale = condition_scale, rescaled_phi = rescaled_phi)
    ...
    def p_mean_variance(self, x, t, classes, condition_scale, rescaled_phi, clip_denoised = True):
        preds = self.model_predictions(x, t, classes, condition_scale, rescaled_phi)
    ...
    @torch.no_grad()
    def p_sample(self, x, t: int, classes, condition_scale = 6., rescaled_phi = 0.7, clip_denoised = True):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full((x.shape[0],), t, device = x.device, dtype = torch.long)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(x = x, t = batched_times, classes = classes, condition_scale = condition_scale, rescaled_phi = rescaled_phi, clip_denoised = clip_denoised)
        noise = torch.randn_like(x) if t > 0 else 0. # no noise if t == 0
        pred_img = model_mean + (0.5 * model_log_variance).exp() * noise
        return pred_img, x_start

    @torch.no_grad()
    def p_sample_loop(self, classes, shape, condition_scale = 6., rescaled_phi = 0.7):
        _, device = shape[0], self.betas.device

        img = torch.randn(shape, device=device)

        for t in tqdm(reversed(range(0, self.num_timesteps)), desc = 'sampling loop time step', total = self.num_timesteps):
            img, x_start = self.p_sample(img, t, classes, condition_scale, rescaled_phi)

        img = unnormalize_to_zero_to_one(img)
        return img

    @torch.inference_mode()
    def ddim_sample(self, classes, shape, condition_scale = 6., rescaled_phi = 0.7, clip_denoised = True):
        ...
        for time, time_next in tqdm(time_pairs, desc = 'sampling loop time step'):
            ...
            pred_noise, x_start, *_ = self.model_predictions(img, time, classes, condition_scale = condition_scale, rescaled_phi = rescaled_phi, clip_x_start = clip_denoised)
    ...
    @torch.inference_mode()
    def sample(self, classes, condition_scale = 6., rescaled_phi = 0.7):
        batch_size, image_size, channels = classes.shape[0], self.image_size, self.channels
        sample_fn = self.p_sample_loop if not self.is_ddim_sampling else self.ddim_sample
        return sample_fn(classes, (batch_size, channels, image_size, image_size), condition_scale, rescaled_phi)
...
num_classes = 10
from base_models.complex_unet_classifier_free_guidance import UnetClassifierFree
model = UnetClassifierFree(
    dim = 64,
    num_classes = num_classes,
    condition_class_drop_prob = 0.5
)
